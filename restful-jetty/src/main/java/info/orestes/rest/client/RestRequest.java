...
public class RestRequest extends HttpRequest {
    ...
    public Request content(ContentProvider content, String contentType) {
        if (content instanceof EntityContentProvider) {
            ...
            if (contentType != null) {
                entityContent.setMediaType(MediaType.parse(contentType));
            }
        }
    ...
    public void send(CompleteListener listener) {
        if (listener instanceof ResponseListener<?>) {
            ...
            for (org.apache.tika.mime.MediaType mediaType : getClient().getConverterService().getAcceptableMediaTypes(entityType)) {
